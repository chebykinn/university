...
int get_proc_num_from_args(int argc, char *const argv[], void *data) {
    ...
    struct option long_opts[] = {{"mutexl", no_argument, (int *)data, 1},
                                 {0, 0, 0, 0}};
...
int child_handle_cs(IOHandle *handle) {
    int rc;
    Message msg;
    rc = receive_any(handle, &msg);
    if (rc != 0) {
    ...
    switch (msg.s_header.s_type) {
        case CS_REQUEST:
            fork_data->reqf[handle->last_msg_pid] = 1;
            assert(fork_data->clean[handle->last_msg_pid]);
            if (!fork_data->clean[handle->last_msg_pid] ||
                !fork_data->dirty[handle->last_msg_pid]) {
                break;
            }
            fork_data->clean[handle->last_msg_pid] =
                fork_data->dirty[handle->last_msg_pid] = 0;
            move_time();
            ...
            break;
        case CS_REPLY:
            fork_data->clean[handle->last_msg_pid] = 1;
            fork_data->dirty[handle->last_msg_pid] = 0;
            break;
        case DONE:
            handle->done_cnt++;
